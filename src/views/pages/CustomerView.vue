...
const headers = [
    ...
    { title: 'Членство', key: 'membership', algin: 'start' },
    { title: 'Награды', key: 'rewards', algin: 'end' },
    { title: 'Ожидаемые продажи', key: 'hasItemInShoppingCart', algin: 'start' },
...
function onConfirm() {
    if (selectedId.value) {
        customerStore
            .delteCustomer(selectedId.value)
            .then((res) => {
                dialog.value = false;
                if (res.status) {
                    notice.value = true;
...
<template>
<!--    <UiMainContainer>-->
        <UiParentCard title="Покупатели" :createFn="onCreate">
